...
for i,x in enumerate(atom):
  if x[0] != -10000:
    if system_is_charged:
      fmt = "    pair_coeff @atom:{0} @atom:{0} lj/cut/coul/long {1} {2}\n"
    else:
      fmt = "    pair_coeff @atom:{0} @atom:{0} {1} {2}\n"
...
if system_is_charged:
  pair_style = "    pair_style hybrid lj/cut/coul/long 10.0 10.0\n"
  kspace_style = "    kspace_style pppm 0.0001\n"
else:
  pair_style = "    pair_style lj/cut 10.0\n"
  kspace_style = ""
